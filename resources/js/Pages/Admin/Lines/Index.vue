...
function handleDelete() { if (deleteId.value) router.delete(`/admin/lines/${deleteId.value}`); deleteDialog.value = false; }
function handleBulkAction(action: string, ids: number[]) { router.post('/admin/lines/mass-action', { action, ids }); }
...
<template>
    ...
    <AdminLayout>
        ...
        <DataTable :columns="columns" :rows="lines.data" :total="lines.total" :current-page="lines.current_page" :last-page="lines.last_page" :per-page="lines.per_page" :from="lines.from" :to="lines.to" route-name="admin.lines.index" :filters="filters" @bulk-action="handleBulkAction">
            <template #filters>
                <Select :model-value="filters.status || ''" class="w-32" @update:model-value="v => router.get('/admin/lines', { ...filters, status: v || undefined, page: 1 }, { preserveState: true })">
            ...
            <template #actions="{ selected, bulkAction }">
                ...
                <Link href="/admin/lines/create"><Button size="sm"><Plus class="mr-1 h-4 w-4" />Add Line</Button></Link>
            </template>
            ...
            <template #row-actions="{ row }">
                <div class="flex items-center justify-end gap-1">
                    <Link :href="`/admin/lines/${row.id}/edit`"><Button variant="ghost" size="icon" class="h-8 w-8"><Pencil class="h-4 w-4" /></Button></Link>
