...
const form = useForm({
    username: props.line.username, password: props.line.password,
    exp_date: props.line.exp_date ? props.line.exp_date.split('T')[0] : '',
    max_connections: props.line.max_connections,
    is_trial: Boolean(props.line.is_trial), admin_enabled: Boolean(props.line.admin_enabled),
    bouquet: props.line.bouquet || [],
    package_ids: (props.line.packages || []).map((p: any) => p.id),
});

function submit() { form.put(`/reseller/lines/${props.line.id}`); }
</script>

<template>
    <Head :title="`Edit: ${line.username}`" />
    <ResellerLayout>
        <template #title>Edit Line</template>
        <div class="mx-auto max-w-2xl">
            <div class="mb-6"><Link href="/reseller/lines" class="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft class="h-4 w-4" /> Back</Link></div>
            <Card class="p-6">
                <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div class="space-y-2 sm:col-span-2">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <label v-for="pkg in packages" :key="pkg.id" class="flex items-center gap-2 rounded-lg border px-3 py-2 cursor-pointer hover:bg-muted/50 transition-colors" :class="{ 'border-primary bg-primary/5': form.package_ids.includes(pkg.id) }">
                                    <input type="checkbox" :value="pkg.id" v-model="form.package_ids" class="rounded border-input" /><span class="text-sm">{{ pkg.package_name }}</span>
                                </label>
                            </div>
                        </div>
                        <div class="space-y-2 sm:col-span-2">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <label v-for="b in bouquets" :key="b.id" class="flex items-center gap-2 rounded-lg border px-3 py-2 cursor-pointer hover:bg-muted/50 transition-colors" :class="{ 'border-primary bg-primary/5': form.bouquet.includes(b.id) }">
                                    <input type="checkbox" :value="b.id" v-model="form.bouquet" class="rounded border-input" /><span class="text-sm">{{ b.bouquet_name }}</span>
                                </label>
                            </div>
                        </div>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t"><Link href="/reseller/lines"><Button variant="outline" type="button">Cancel</Button></Link><Button type="submit" :disabled="form.processing">Save Changes</Button></div>
                </form>
            </Card>
        </div>
    </ResellerLayout>
