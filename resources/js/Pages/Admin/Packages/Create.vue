...
const form = useForm({ package_name: '', is_trial: false, is_official: true, is_addon: false });
function submit() { form.post('/admin/packages'); }
</script>

<template>
    <Head title="Add Package" />
    <AdminLayout>
        <template #title>Add Package</template>
        <div class="mx-auto max-w-lg">
            ...
            <Card class="p-6">
                <form @submit.prevent="submit" class="space-y-4">
                    <div class="space-y-2"><label class="text-sm font-medium">Name *</label><Input v-model="form.package_name" /><p v-if="form.errors.package_name" class="text-xs text-destructive">{{ form.errors.package_name }}</p></div>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t"><Link href="/admin/packages"><Button variant="outline" type="button">Cancel</Button></Link><Button type="submit" :disabled="form.processing">Create</Button></div>
                </form>
            </Card>
        </div>
    </AdminLayout>
